...
def get_current_customer_id(credentials: HTTPAuthorizationCredentials = Depends(customer_oauth)) -> int:
    ...
    """
    customer_id = verify_customer_bearer_token(credentials.credentials)
    if customer_id is None:
        raise unauthorized("Invalid or expired customer token")
    return customer_id


def get_current_admin_id(credentials: HTTPAuthorizationCredentials = Depends(admin_oauth)) -> bool:
    ...
    HTTPBearer returns HTTPAuthorizationCredentials with a .credentials attribute.
    """
    is_admin = verify_admin_bearer_token(credentials.credentials)
    if not is_admin:
        raise forbidden("Admin credentials required")
    return True


def get_current_carrier_key(api_key_value: str = Depends(carrier_api_key)) -> str:
    ...
    """
    if not verify_carrier_api_key(api_key_value):
        raise unauthorized("Invalid carrier API key")
    return api_key_value


def get_customer_or_admin(authorization: str | None = Header(None)) -> dict:
    ...
    customer_id = verify_customer_bearer_token(token)
    if customer_id is not None:
        return {"type": "customer", "id": customer_id}

    is_admin = verify_admin_bearer_token(token)
    if is_admin:
        return {"type": "admin"}
